	...
	<body onload="processerbar(3000)">
					...
					<div class="r-t-nav-main clearfix">
						...
						<div class="three-mun-box left">
							<ul class="clearfix">
								...
								<li><a href="javascript:;" class="active" >未审核视频(${count_videos[3]})</a></li>
										...
										<div class="manager_tool">
											<div class="manager_action">
												...
												<div class="adddel" id="adddel">
														...
														<div class="fn2nd">
															<a href="javascript:;" onclick="deleteAllVideoByVID(${user.userid},0)">删除</a>
														</div>
												</div>
											</div>
											<div class="clear"></div>
												...
												<td class="m_stat">
													...
													<td class="m_opt">
														<div class="action">
															<c:choose>
															<c:when test="${video.tags eq null || video.descript eq null || video.categorys eq null || video.coursename eq null }">
															<span class="share"><a href="javascript:;" onclick="deleteVideoByVID(${video.videoid},${user.userid},${page},0,11,${video.audit})">删除</a></span>
															</c:when>
															<c:otherwise>
															<span class="share"><a href="javascript:;" onclick="deleteVideoByVID(${video.videoid},${user.userid},${page},0,${video.audit}),111">删除</a></span>
															</c:otherwise>
															</c:choose>
		 ...
		 <div class="auto-1467597023087">
		         ...
		         <c:when test="${page -1 > 0 }">
		         	<a class="zbtn zprv" href="javascript:;" onclick="findNVByPage(${user.userid},${page-1})">上一页</a>
		         </c:when>
		         <c:otherwise>
				 ...
				 <c:when test="${maxPage>6}">
				     ...
				     <c:when test="${page<6}">
					         ...
					         <c:otherwise>
					         	<a class="zpgi" href="javascript:;" onclick="findNVByPage(${user.userid},${x})">${x}</a>
					         </c:otherwise>
					         </c:choose>
							 </c:forEach>
					 ...
					 <c:when test="${page>=maxPage-4}">
					         <a class="zpgi" href="javascript:;" onclick="findNVByPage(${user.userid},1)">1</a>
					 		 <span class="zdot">...</span>
				...
				<c:otherwise>
					 ...
					 <c:choose>
					 <c:when test="${x==page}">
			         	<a class="zpgi js-selected" href="javascript:;" onclick="findNVByPage(${user.userid},${x})">${x}</a>
			         </c:when>
			         <c:otherwise>
			         	<a class="zpgi" href="javascript:;" onclick="findNVByPage(${user.userid},${x})">${x}</a>
			         </c:otherwise>
		         ...
		         <c:when test="${page < maxPage }">
		         	<a class="zbtn znxt" href="javascript:;" onclick="findNVByPage(${user.userid},${page+1})">下一页</a>
		         </c:when>
		         <c:otherwise>
