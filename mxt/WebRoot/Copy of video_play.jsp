	...
	<img src="images/logo.png">
	 ...
	 <div style="margin:0 0 10px 0;width:100%;">
		 <div style="width:875px;">
			<h1><%=video.getTitle()%></h1><span class="jb"><a href="">举报</a></span>
		 </div>
	 </div>
	...
	function ckadjump(){
		...
		<div class="bfr clear">
			...
			<ul id="ke_list" class="kcml">
				...
				<li style="margin:10px 0px;">
						...
						<span>${fn:substring(chapter.chapterName,0,15)}...</span>
						</c:when>
						<c:otherwise>
						...
						<i ></i>
									<c:choose>
									<c:when test="${fn:length(vi.title)>12}">
									<font color="#72d0eb" >${fn:substring(vi.title,0,12)}...</font>
									</c:when>
									<c:otherwise>
						...
						<a href="video/play.do?id=${vid.videoid}"> ${vid.title}</a>
						            ...
						            <c:choose>
									<c:when test="${fn:length(vi.title)>12}">
									<a href="video/play.do?id=${vi.videoid}">${fn:substring(vi.title,0,12)}...</a>
									</c:when>
									<c:otherwise>
									<a href="video/play.do?id=${vi.videoid}"> ${vi.title}</a>
									</c:otherwise>
									</c:choose>
	 ...
	 <div style="width:875px;margin-top:70px;float:left;">
		 ...
		 <div class="plq">
			...
			<div><a href="#" class="ptit">张美辰1732</a></div>
			<div>
				<form action="">
					<textarea onFocus="if(this.value=='看点槽点，不吐不快，别憋着，马上大声说出来吧！'){this.value='';}"  onblur="if(this.value==''){this.value='看点槽点，不吐不快，别憋着，马上大声说出来吧！';}" class="form_input" name="content" id="content" cols="30" rows="10">看点槽点，不吐不快，别憋着，马上大声说出来吧！</textarea>
				<input class="asx fr" type="submit" value="发表评论">
				</form>
			</div>
		 </div>
		 <!--评论内容表头-->
